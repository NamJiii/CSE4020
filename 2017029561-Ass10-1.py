...
def render():
    ...
    # ZYX Euler angles
    z1ang = np.radians(alpha)
    xang = np.radians(beta)
    z2ang = np.radians(gamma)
    M = np.identity(4)
    Rx = np.array([[np.cos(z1ang), -np.sin(z1ang), 0],
                   [np.sin(z1ang), np.cos(z1ang), 0],
                   [0,0,1]])
    Ry = np.array([[1,0,0],
                   [0, np.cos(xang), -np.sin(xang)],
                   [0, np.sin(xang), np.cos(xang)]])
    Rz = np.array([[np.cos(z2ang), -np.sin(z2ang), 0],
...
def main():
    ...
    if not glfw.init():
        return
    window = glfw.create_window(480,480,'2017029561-Ass10-1', None,None)
    if not window:
        glfw.terminate()
        return
...
def render():
    global gCamAng, gCamHeight, alpha
    glClear(GL_COLOR_BUFFER_BIT|GL_DEPTH_BUFFER_BIT)
    glEnable(GL_DEPTH_TEST)
